...
// --------------------------------------------------------------------
	 ...
	 * @return	bool
		...
		this part is created and modified by PG
	 */
	public function do_url_upload($_url = '')
	{
		echo 'doing url upload';
		//check the url and save the ext name of file as $PG_ext 
		...
		// let's fetch the image, the limit of image is 10MB
		$f = file_get_contents($_url, NULL, NULL, NULL, 10*1024*1024);
		if ($f == false) die('抓取檔案失敗');
		$r = file_put_contents('pic/file_test_PG.'.$PG_ext, $f);
		
		// Set the uploaded data as class variables
		$this->file_temp = 'pic/file_test_PG.'.$PG_ext;
		$this->file_size = $r;
		$this->_file_mime_type($PG_ext);
		$this->file_type = preg_replace("/^(.+?);.*$/", "\\1", $this->file_type);
		$this->file_type = strtolower(trim(stripslashes($this->file_type), '"'));
		//$this->file_name = $this->_prep_filename($_FILES[$field]['name']);
		$this->file_ext	 = '.'.$PG_ext;
		$this->client_name = 'download by PG';
		 ...
		 * reliably work in most environments
		 */
		echo 'moving '.$this->file_temp.' to '.$this->upload_path.$this->file_name.'<br />';
		if ( ! @copy($this->file_temp, $this->upload_path.$this->file_name))
		{
			if ( ! @move_uploaded_file($this->file_temp, $this->upload_path.$this->file_name))
			{
